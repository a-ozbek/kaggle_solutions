...
# Train
X = np.concatenate([func(X) for func in aug_funcs], axis=0)
y = np.concatenate([y] * len(aug_funcs))
...
df_sub_xgb.to_csv('./ahmetsmodels_xgboost5FOLD_test'+ '.csv', index=False)
df_sub_lr.to_csv('./ahmetsmodels_logreg5FOLD_test'+ '.csv', index=False)
